...
TRAVEL.HUNGER_COST = GetModConfigData("HungerCost")
TRAVEL.SANITY_COST = GetModConfigData("SanityCost")
TRAVEL.COUNTDOWN_ENABLE = GetModConfigData("CountdownEnable")

if GLOBAL.KnownModIndex:IsModEnabled("workshop-1416161108") then
    print("Camp Security is enabled.")
    TRAVEL.CampSecurityCheckPermissionUse = function(doer, target)
        return not Ownership or GLOBAL.checkPermissionUse(doer, target)
    end
else
    print("Camp Security is NOT enabled.")
    TRAVEL.CampSecurityCheckPermissionUse = function(doer, target)
        return true
    end
end
...
GLOBAL.ACTIONS.DESTINATION_UI.mount_valid = true
...
AddComponentAction("SCENE", "travelable", function(inst, doer, actions, right)
    if right then
        if not inst:HasTag("burnt") and not inst:HasTag("fire") and
            TRAVEL.CampSecurityCheckPermissionUse(doer, inst)
        then
